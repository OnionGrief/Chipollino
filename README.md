...
**ВАЖНО!** Для корректного составления *latex*-файла нужно поставить [Graphviz](https://graphviz.org/download/), чтобы в системе поддерживалась команда `dot`. Без этого не будет работать генерация картинок.

Если в системе поддерживается команда `pdflatex`, то тогда сразу будет генерироваться *pdf*-документ. Если нет, его можно собрать самому.
...
Для функций и последовательностей функций должны выполняться соответствия типов. Больше про типы фунцкий - ниже в разделе "Функции преобразователя"
...
- `DeLinearize: Regex -> Regex` — снимает разметку
- `DeAnnote: Regex -> Regex` — снимает разметку
...
- `DeAnnote: NFA -> NFA` — снимает разметку
- `DeLinearize: NFA ->NFA`
...
* `Test: (Regex, Regex, Int) -> таблица` — порождает слова, принадлежащие второму языку (2 аргумент), раскрывая каждую итерацию Клини начиная с 0 (пустого слова) и увеличивая с каждым разом на заданное значение (3 аргумент). Если в регулярном выражении (для 2 языка) присутствуют альтернативы, то выбирается 1ая из них.
То есть
`a*b` c шагом 2 раскрывается в ряд: b, aab, aaaab ...
`(a*b)*` c шагом 1 раскрывается в ряд: c, abc, aabaabc ...)
...
* `Test : (NFA, Regex, Int) -> таблица` — аналогичен методу для регулярного выражения.


`NFA` здесь можно просто понимать как `FA`, для которого не обязан быть включённым флаг детерминизма.
