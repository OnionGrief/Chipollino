...
TEST(TestCaseName, Test_remove_trap) {
	...
	ASSERT_TRUE(fa4.size() == 1); /* Кейс, когда осталось несколько ловушек, и они коллапсируют в одну, чтобы не получился пустой автомат. */
	ASSERT_TRUE(fa3.size() == fa3.remove_trap_states().size()); 
	/* Кейс, когда ловушек нет. */
}
